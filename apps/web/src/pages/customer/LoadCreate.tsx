      ...
      {step === 1 ? (
        <div className="card form-grid">
          ...
          <label className="form-group">
            ...
            <input className="form-input" list="city-list" value={form.fromCity} onChange={(e) => { update('fromCity', e.target.value); update('fromDistrict', ''); setFromNeighborhood('') }} />
          </label>
          <label className="form-group">
            <span className="form-label">Çıkış İlçesi</span>
            <select className="form-input" value={form.fromDistrict} onChange={(e) => { update('fromDistrict', e.target.value); setFromNeighborhood('') }}>
              <option value="">İlçe seçin</option>
              {getDistrictsByCity(form.fromCity).map((district) => <option key={district} value={district}>{district}</option>)}
            </select>
          </label>
          <label className="form-group">
            ...
            <select className="form-input" value={fromNeighborhood} onChange={(e) => setFromNeighborhood(e.target.value)}>
              ...
              {getNeighborhoodsByDistrict(form.fromDistrict).map((hood) => <option key={hood} value={hood}>{hood}</option>)}
            </select>
          </label>
          <label className="form-group">
            <span className="form-label">Varış Şehri</span>
            <input className="form-input" list="city-list" value={form.toCity} onChange={(e) => { update('toCity', e.target.value); update('toDistrict', ''); setToNeighborhood('') }} />
          </label>
          <label className="form-group">
            <span className="form-label">Varış İlçesi</span>
            <select className="form-input" value={form.toDistrict} onChange={(e) => { update('toDistrict', e.target.value); setToNeighborhood('') }}>
              <option value="">İlçe seçin</option>
              {getDistrictsByCity(form.toCity).map((district) => <option key={district} value={district}>{district}</option>)}
            </select>
          </label>
          <label className="form-group">
            ...
            <select className="form-input" value={toNeighborhood} onChange={(e) => setToNeighborhood(e.target.value)}>
              <option value="">Mahalle seçin</option>
              {getNeighborhoodsByDistrict(form.toDistrict).map((hood) => <option key={hood} value={hood}>{hood}</option>)}
            </select>
          </label>
          <label className="form-group">
            <span className="form-label">Alım Tarihi</span>
            <TurkishDateTimePicker value={form.pickupDate} onChange={(iso) => update('pickupDate', iso)} onClear={() => update('pickupDate', new Date().toISOString())} />
          </label>
          <label className="form-group">
            <span className="form-label">Teslim Tarihi</span>
            <TurkishDateTimePicker value={form.deliveryDate} onChange={(iso) => update('deliveryDate', iso)} onClear={() => update('deliveryDate', new Date().toISOString())} />
          </label>
          <label className="form-group" style={{ gridColumn: '1 / -1' }}>
          ...
          {saveAddress ? (
            <label className="form-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <input className="form-input" value={savedAddressName} onChange={(e) => setSavedAddressName(e.target.value)} placeholder="Örn: Fabrika Depo 1" />
            </label>
          ) : null}
          <datalist id="city-list">
            {TR_CITIES.map((city) => <option key={city} value={city} />)}
          </datalist>
        </div>
      ) : null}
